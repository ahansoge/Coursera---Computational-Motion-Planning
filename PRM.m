...
for i = 2:nsamples
    ...
    %
    [dist_sort,dist_ind] = sort(distances);
    
     k_ind = min(k,size(dist_ind));
     
     for j=1:k_ind
         if(LocalPlanner(x,samples(:,dist_ind(j))))
             nedges=nedges+1;
             edges(nedges,:) = [i,dist_ind(j)];
             edge_lengths(nedges) = dist_sort(j);
         end
     end
